locals {
  ...
  name                   = [for i in range(local.node_pool_count) : local.predifined_macine_type ? format("%s%s", var.preemtible ? "preemtible-" : "", var.machine_type) : format("%s%s", local.name_gpu_prefix[i], local.machine_type[i])]
